...
export default function AppBackgroundSection({ token, appBg, setAppBg, showToast, dark }) {
  ...
  const tabBtn = (key, label) => (
    <button
      ...
      className={
        tab === key
          ? "flex-1 px-3 py-1.5 text-sm rounded-lg font-semibold transition-all duration-200 bg-gradient-to-r from-indigo-500 to-violet-600 text-white hover:from-indigo-600 hover:to-violet-700 shadow-md shadow-indigo-300/40 dark:shadow-none hover:shadow-lg hover:shadow-indigo-300/50 dark:hover:shadow-none hover:scale-[1.03] active:scale-[0.98] btn-gradient"
          : "flex-1 px-3 py-1.5 text-sm rounded-lg font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"
      }
    >
      {label}
    </button>
