...
export default function MultiSelectToolbar({
  ...
  const renderActionButton = (action, opts = {}) => {
    ...
    if (action.kind === "cta") {
      ...
      const ctaFull =
        "h-9 px-3.5 inline-flex items-center justify-center gap-1.5 rounded-lg text-sm font-bold whitespace-nowrap shrink-0 text-white bg-gradient-to-r from-indigo-600 to-violet-700 ring-1 ring-violet-500/30 transition-all duration-200";
      const ctaCompact =
        "h-9 w-9 inline-flex items-center justify-center rounded-lg text-sm font-bold shrink-0 text-white bg-gradient-to-r from-indigo-600 to-violet-700 ring-1 ring-violet-500/30 transition-all duration-200";
      const ctaActive =
